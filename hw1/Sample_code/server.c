...
int main(int argc, char** argv) {
    int i, ret, process = -1,flag = 0,x = 0;
    ...
    while (1) {
        ...
        readfdes = master;
        if(select(maxfd,&readfdes,NULL,NULL,&tv) <= 0){
            continue;
        }
        for(int i = 3;i < maxfd;i ++){
            if(FD_ISSET(i,&readfdes)){
                    if(i == svr.listen_fd){
                        ...
                        if(filearray[conn_fd] != 0)
                            file_fd = filearray[conn_fd];
                        // requestP[conn_fd]->filename is guaranteed to be successfully set. 
                        ...
                        if (file_fd == -1) {
                            ...
                            S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
                                ...
                                if(strcmp(array[j],requestP[conn_fd].filename) == 0 && j != conn_fd){
                                    write(requestP[conn_fd].conn_fd, reject_header, sizeof(reject_header));
                                    if (file_fd >= 0) close(file_fd);
                                    FD_CLR(conn_fd, &master);
                                    close(requestP[conn_fd].conn_fd);
                            ...
                            if(x == 1){
                                ...
                                strcpy(array[conn_fd],requestP[conn_fd].filename);
                            }*/
                            filearray[conn_fd] = file_fd;
                            // TODO: Add lock
                            lock.l_type = F_WRLCK;
                            lock.l_start = 0;
                            lock.l_whence = SEEK_SET;
